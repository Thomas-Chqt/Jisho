...
public class Mot: Entity {
	...
	var listesIn: Set<Liste> {
		get {
			guard let listesIn_atb = listesIn_atb else { return [] }
			guard let listesIn = listesIn_atb as? Set<Liste> else { return [] }
			return listesIn
		}
		set {
			if newValue.isEmpty { listesIn_atb = nil ; return}
			listesIn_atb = NSSet(set: newValue)
		}
	}
